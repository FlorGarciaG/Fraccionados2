...
const EditProblems = () => {
    ...
    const fetchData = async () => {
        try {
            const res = await fetch('http://localhost:3000/api/problemsEs', {
                cache: 'no-store',
            });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEditing) {
                await fetch(`/api/problemsEs/${editingProblemId}`, {
                    method: 'PUT',
                    headers: {
    ...
    const handleEdit = async (id) => {
        try {
            const res = await fetch(`/api/problemsEs/${id}`);
            const data = await res.json();
